...
if [ $DOCKER -eq 1 ]; then
    ...
    git archive --format=tar.gz -o Calendar.tar.gz --prefix Calendar/ HEAD
    if [ -d ${BUILD_LINUX_DIR} ]; then
        rm -fr ${BUILD_LINUX_DIR}
    fi
    mkdir -p ${BUILD_LINUX_DIR}
    mv Calendar.tar.gz ${BUILD_LINUX_DIR}/
    ...
    if [ $DEB -eq 1 ]; then
        ...
        docker run --privileged ${DOCKER_PARA} \
           ...
           bash -e -x -c "
           if [ ! \$CI ]; then
               tar -C ~ -xf /home/build/Calendar.tar.gz
    ...
    if [ $APPIMAGE -eq 1 ]; then
        ...
        docker run --privileged ${DOCKER_PARA} \
            ...
            \${SOURCE_CODE_DIR}/Calendar/Script/build_linux.sh --appimage --install=/home/install --tools=/home/tools --verbose=${BUILD_VERBOSE}
    ...
    if [ $RPM -eq 1 ]; then
        docker run --volume ${BUILD_LINUX_DIR}:/home/build \
            ...
            cp /home/build/Calendar.tar.gz ~/rpmbuild/SOURCES/
            tar -C ~ -xf /home/build/Calendar.tar.gz
            # Install getopt
            dnf install -y util-linux
            ~/Calendar/Script/build_linux.sh --rpm --install=/home/install --tools=/home/tools --verbose=${BUILD_VERBOSE}
            cp ~/rpmbuild/RPMS/`uname -m`/calendar*.rpm /home/build/
            "
    fi
    exit 0
fi
...
#        esac
    ...
    fedora)
        ...
        #dnf builddep -y ${REPO_ROOT}/Package/rpm/calendar.spec
        ;;
    *)
    esac
...
if [ $MACOS -eq 1 ]; then
    ...
    ./build_depend.sh --system_update --base --default --macos --rabbitcommon \
        --install=${INSTALL_DIR} \
        --source=${SOURCE_DIR} \
        --tools=${TOOLS_DIR} \
        --verbose=${BUILD_VERBOSE}
