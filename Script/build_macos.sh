...
cmake "$REPO_ROOT" \
  -DCMAKE_INSTALL_PREFIX=Calendar.App/Contents \
...
if [ -d "${MACDEPLOYQT}" ]; then
    echo "macdeployqt ......"
    ${MACDEPLOYQT} `pwd`/Calendar.App -dmg -verbose=3
else
    echo "Error: Don't found macdeployqt, please set QT_ROOT"
fi

7z a Calendar.zip ./Calendar.App

if [ -f Calendar.zip ]; then
    echo "--- Generated zip: $REPO_ROOT/Calendar.zip"
    cp Calendar.zip $REPO_ROOT/
fi
if [ -f Calendar.dmg ]; then
    echo "--- Generated dmg: $REPO_ROOT/Calendar.dmg"
    cp Calendar.dmg $REPO_ROOT/
fi
